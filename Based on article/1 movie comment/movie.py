...
与词根还原不同(stemmer)，后者是抽取一个单词的词根。由于处理的是
字符串，我们首先要想方法把字符串转换为向量/数字表示。一种解决方
法是可以把单词映射为数字ID。

第二个问题是每行评论字数不同，而神经网络需要一致的输入(其实有些
神经网络不需要，至少本帖需要)，这可以使用词汇表解决。
"""
...
def create_lexicon(pos_file, neg_file):
	...
	def process_file(f):
		with open(pos_file, 'r') as f:
			...
			#print(lines)
			for line in lines:
				words = word_tokenize(line.lower())#将字母都以小写表示
				lex += words#将分解后的词语都放到lex数组中去
			return lex
...
def train_neural_network(X, Y):
 ...
 #使用feed_dict填充上面占位符X,Y未赋的值，并计算cost_func
		...
		#eval() 其实就是tf.Tensor的Session.run() 的另外一种写法，重新走了上面
		#的一个流程，
