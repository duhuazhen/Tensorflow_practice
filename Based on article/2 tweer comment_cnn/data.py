			...
			for line in f:
				...
				for word in words:
					...
					if word not in count_word:
						...
						count_word[word] += 1
 #count_word.items()得到的[(键，值)]的列表， 通过sorted方法，指定排序的键值key是原来字典中的value属性，其中用到了匿名函数lambda， 
 #参数为t列表，返回第二个元素t[1]，也就是每个键值对中的value，  从小到大排序时 reverse=False，从大到小排序是True！
...
#　序列化对象，并将结果数据流写入到文件对象中。参数protocol是序列化模式，默认值为0，表示以文本的形式序列化。protocol的值还可以是1或2，
#表示以二进制的形式序列化。
